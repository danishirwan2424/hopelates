    ...
    <DonorNav />
      ...
      <div className="flex-grow bg-white p-4 sm:p-10 rounded-xl sm:rounded-l-2xl shadow-xl">
        ...
        <form className="space-y-12">
          ...
          {currentStep === 1 && (
            <section>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {packages.map(pkg => (
                  <div key={pkg.id} className="bg-white rounded-xl shadow hover:shadow-lg transition p-6 flex flex-col items-center">
                    ...
                    <div className="flex items-center gap-3 mt-auto">
                      ...
                      <span className="font-medium">{packageQuantities[pkg.id]}</span>
                      <button
                        type="button"
                        onClick={() => handleQuantityChange(pkg.id, 1)}
              ...
              <div className="mt-6 flex justify-between items-center">
                ...
                {getTotalItems() > 0 ? (
                  <div className="bg-gray-100 rounded-xl p-4 flex items-center">
                    ...
                    <span className="font-bold ml-4">Total: RM {calculateTotal()}</span>
                  </div>
                ) : (
                  <div></div> // keep spacing if no items selected
                )}

                {/* Buttons always on right */}
          ...
          {currentStep === 3 && (
            <section>
              <h2 className="text-xl font-semibold pl-4 border-l-4 mb-4" style={{ borderColor: primaryColor, color: darkColor }}>Payment</h2>
              <div
                {...getRootProps()}
                className={`w-full h-32 border-2 border-dashed rounded-lg flex flex-col items-center justify-center cursor-pointer transition ${
              ...
              >
                ...
                {uploadedFile ? (
                  ...
                  <p className="text-gray-500">Drop file here or click to upload (PNG, JPEG, PDF, max 5MB)</p>
                )}
              </div>
