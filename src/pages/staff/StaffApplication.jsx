...
function StaffApplication() {
  ...
  const fetchApplications = async () => {
    try {
      ...
      const rows = res.data?.data || []; // ✅ backend now returns {success,data}
  ...
  const openEditModal = async (user) => {
    try {
      const res = await axios.get(`http://localhost:5000/api/application/${user.id}`);
  ...
  const updateStatus = async (id, newStatus) => {
    await axios.patch(`http://localhost:5000/api/application/${id}/status`, {
      status: newStatus,
    });
    ...
    if (selectedUser?.id === id) setSelectedUser((p) => ({ ...p, status: newStatus }));
  };
...
=======
            <p className="text-[12px] text-black/50">
              Review and manage food aid applications
>>>>>>> Stashed changes
            </p>
          </header>
          ...
          <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-3 flex-shrink-0">
            <div className="bg-gradient-to-b from-[#11452E] to-[#278659] rounded-2xl shadow-md flex flex-col items-start justify-start h-[150px] p-4">
              ...
              <h2 className="text-[56px] text-white font-bold leading-none">{pendingCount}</h2>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-md flex flex-col items-start justify-start h-[150px] p-4">
              ...
              <h2 className="text-[56px] text-black font-bold leading-none">{approvedCount}</h2>
            </div>

            <div className="bg-white rounded-2xl shadow-md flex flex-col items-start justify-start h-[150px] p-4">
              <p className="text-[14px] text-black/80 mb-2">Rejected</p>
              <h2 className="text-[56px] text-black font-bold leading-none">{rejectedCount}</h2>
            </div>
          </section>
              ...
              <table className="min-w-full text-sm text-left border-collapse">
                <thead className="bg-gray-100 text-gray-700 sticky top-0 z-10">
                  <tr className="text-[12px] uppercase tracking-wide">
                    <th className="py-3 px-4 cursor-pointer whitespace-nowrap" onClick={() => handleSort("applicationId")}>
                      Application ID {sortBy === "applicationId" ? (sortOrder === "asc" ? "▲" : "▼") : ""}
                    </th>
                    <th className="py-3 px-4 cursor-pointer whitespace-nowrap" onClick={() => handleSort("name")}>
                      Name {sortBy === "name" ? (sortOrder === "asc" ? "▲" : "▼") : ""}
                    </th>
                    <th className="py-3 px-4 cursor-pointer whitespace-nowrap" onClick={() => handleSort("email")}>
                      Email {sortBy === "email" ? (sortOrder === "asc" ? "▲" : "▼") : ""}
                    </th>
                    <th className="py-3 px-4 cursor-pointer whitespace-nowrap" onClick={() => handleSort("date")}>
                      Date Applied {sortBy === "date" ? (sortOrder === "asc" ? "▲" : "▼") : ""}
                    </th>
                    <th className="py-3 px-4 cursor-pointer whitespace-nowrap" onClick={() => handleSort("status")}>
                ...
                <tbody>
                  {filteredApps.map((app) => (
                    <tr key={app.id} className="hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-b-0">
                      <td className="py-3 px-4 font-medium text-gray-900 whitespace-nowrap">{app.applicationId}</td>
                      <td className="py-3 px-4 font-medium text-gray-900">{app.name}</td>
                      <td className="py-3 px-4 text-gray-600">{app.email}</td>
                      <td className="py-3 px-4 text-gray-600 whitespace-nowrap">{app.date}</td>
                  ...
                  {filteredApps.length === 0 && (
                    <tr>
                      <td colSpan={6} className="py-12 text-center text-gray-500">No applications found</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          {showModal && (
            <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
              <div className="bg-white w-[420px] p-6 rounded-2xl shadow-lg relative">
                <button onClick={closeModal} className="absolute right-4 top-4 text-gray-600 hover:text-black">✕</button>
